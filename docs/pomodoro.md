...
Renderer state is stored under `agent-activity.pomodoro`. A running phase stores an absolute `endsAt` Unix timestamp. The UI derives remaining time from `endsAt - Date.now()` on every tick, mount, focus, and visibility change. Reloading, background throttling, or sleeping the Mac therefore does not extend the phase.

Settings are stored separately under `agent-activity.pomodoro-settings`. Each phase snapshots `phaseDurationMs` when it is prepared. Editing settings updates an idle phase immediately, but never changes a running or paused countdown; Restart phase or the next prepared phase uses the new values. **Restart** reloads only the current phase while preserving cycle progress. Confirmed **Reset all** stops the old cycle, returns to idle Focus with zero completed sessions, clears recent completion/Pet state, and preserves custom timer settings. Restoring Defaults remains the separate 25/5/15/every-4 action.
